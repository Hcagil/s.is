...
| --- | --- | --- |
| D1 — in progress | Verify remote CI and finish one-time authorized USB/containerized ADB setup. Install/launch the current build; verify native Google account sign-in once OAuth is configured. | ADB install/launch passed. Play-installed build-1 returned to the sign-in screen after account selection; the app treated the provider's cancellation-class error as a user cancel and hid the reason. Build 2 surfaces the Google/Supabase failure reason on the sign-in and error screens (5 controller tests, analyze/build pass). Suspected cause: no Android OAuth client registered for `com.esd.sis` with the Play App Signing certificate SHA-1 — confirm from the on-screen reason after registering it. Remote CI paths remain unverified. Do not record device identifiers or credentials. |
