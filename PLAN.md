...
| D2 — in progress | Complete Play Console bootstrap for `com.esd.sis`, internal tester access, Play App Signing/upload key, release signing, runtime configuration and publishing API access. Follow [setup prerequisites](docs/SETUP.md#google-play-delivery-bootstrap). | Release builds reject missing signing configuration and a temporary-key signed AAB passed local verification. Play package-name ownership must be confirmed because the unavailable earlier debug key was installed outside Play; real keys, AAB acceptance and Play installation remain pending. |
