...
| --- | --- | --- |
| D1 — pending | Verify remote CI and finish one-time authorized USB/containerized ADB setup. Install/launch the current build; verify native Google account sign-in once OAuth is configured. | Actual CI run results for each change path; phone launch/login result or a precise remaining blocker. Do not record device identifiers or credentials. |
