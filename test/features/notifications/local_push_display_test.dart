...
void main() {
  ...
  /// The preferences file with nothing waiting: only who is signed in.
  Map<String, Object> onlyOwner() => {
    for (final e in disk.values.entries)
      if (e.key == 'flutter.sis.push_inbox_owner') e.key: e.value,
  };
  ...
  group('nobody owns the inbox', () {
    ...
    test('signed out (offline): a push arriving afterwards, in a background '
      ...
      await signedIn();
      await push('c1', 'Ava', 'before sign-out');
      await LocalPushDisplay.forUser(null);
      final before = Map<String, Object>.of(disk.values);

      newIsolate();
      await push('c2', 'Zed', 'after sign-out secret');
    ...
    test('the guard is not vacuous: with an owner the same push draws and '
        'is stored', () async {
      await signedIn();
      newIsolate();
      await push('c1', 'Ava', 'mine');

      expect(shade.childChats, {'c1'});
      expect(jsonEncode(disk.values), contains('mine'));
    });
  });

  group('currentOwner', () {
    test('is null on a phone nobody has signed in on', () async {
      expect(await LocalPushDisplay.currentOwner(), isNull);
    });

    test('is the member forUser was last told, in any isolate', () async {
    ...
    test('is null again after forUser(null)', () async {
      await LocalPushDisplay.forUser('member-a');
      await LocalPushDisplay.forUser(null);
      newIsolate();
      expect(await LocalPushDisplay.currentOwner(), isNull);
  ...
  group('signing out', () {
    ...
    test('empties the shade, and nothing of the last account comes back '
      ...
      // The next one signs in on the same phone. forUser is told about the
      // same member here on purpose: a change of member would clear the
      // inbox itself and hide whether clearAll did.
      await signedIn();
  ...
  group('a change of member (forUser)', () {
    ...
    test('a push delivered late while nobody owns the inbox is not drawn, '
        'and the next start that settles on nobody finds nothing', () async {
      await LocalPushDisplay.forUser('member-a');
      await push('c1', 'Ava', 'secret one');
      await LocalPushDisplay.forUser(null);
      newIsolate(); // FCM delivers late, to a background isolate
      await push('c1', 'Ava', 'late secret');
      expect(shade.posted, isEmpty, reason: 'drawn with no owner');
      expectGone(['late secret']);
