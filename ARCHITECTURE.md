...
Status: initial direction agreed; implementation pending.
...
- **Account deletion:** Remove the Auth account, profile, session/access records and membership; retain messages with a null sender and the label `Deleted user`. Do not retain a hidden sender mapping or claim message bodies are anonymous. Validate retention before store distribution.
