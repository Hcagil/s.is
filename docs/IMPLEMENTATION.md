...
- Use `supabase_flutter` with `google_sign_in` and the native Android Google account flow. Add packages only for required capabilities absent from the current stack; use OS-backed secure session storage. Keep mocks limited to tests and the initial scaffold.
...
- Runtime configuration: `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY` and `GOOGLE_WEB_CLIENT_ID`, supplied outside source control. Use the native Android Google account picker and exchange its tokens with Supabase; do not open browser OAuth. No client service-role key, OAuth secret, signing key or credential logging.
