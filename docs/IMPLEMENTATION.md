...
- Database: clean local migration replay, `supabase db lint`, `supabase test db`. Cover anonymous/unallowlisted access, forged sender/admin operations, concurrent direct creation, membership changes, last-admin succession and account deletion with retained unlinked messages.
...
- Physical Android: Google login/callback, direct/group exchange, background/resume and network interruption. An APK build or mocked login is not device/integration verification.
